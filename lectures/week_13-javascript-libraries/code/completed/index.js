...
app.get('/moment/add', function(req, res){
	...
	if(!days) {
		res.send("please send a valid number of days")
	}
	res.send(moment().add(parseInt(days), 'days').format('MMM Do, YYYY'))
});
...
app.get('/moment/subtract', function(req, res){	
	...
	res.send(moment().subtract(parseInt(days), 'months').format('MMM Do, YYYY'))
});
